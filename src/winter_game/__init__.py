...
def main() -> None:
    ...
    while run:
        ...
        if menu_placement != "main_game":
            for event in pg.event.get():
                ...
                if event.type == pg.QUIT:
                    ...
                    if menu_placement == "load_save":
                        ...
                        menu_placement.split("_")[0] == "save"
                        and len(save_file_name) > 0
                    ):
                        if 200 <= position[1] <= 250:
                            if menu_placement == "save_creation":
                                ...
                                menu_placement = "main_game"
                                with open(
                                    f"src/saves/{save_file_name}.txt",
                                    "r",
                                    encoding="utf-8",
                                ) as file:
                        ...
                        if menu_placement == "options_game":
                            if 0 <= position[1] <= 50:
                                ...
                                with open(
                                    ...
                                    file.write(
                                        f"{chunks_json};{location["tile"]};{tick};{location["room"]}"
                                    )
                                save_file_name = ""
            ...
            inventory = chunks[location["room"]][
            ...
            if (
                not (location["tile"][2], location["tile"][3])
                ...
                ] = chunks[location["room"]][(location["old"][0], location["old"][1])][
                    (location["old"][2], location["old"][3])
                ]
                del chunks[location["room"]][location["old"][0], location["old"][1]][
                    location["old"][2], location["old"][3]
                ]
            else:
                if (
                    chunks[location["room"]][
                        (location["tile"][0], location["tile"][1])
                    ][(location["tile"][2], location["tile"][3])].kind
                    != "player"
                ):
                    location["real"] = [
                        *location["old"],
            ...
            for event in pg.event.get():
                if event.type == pg.QUIT:
                    ...
                    if (
                        abs(position[0] - SCREEN_SIZE[0] / 2 + 32) // (64 * zoom) <= 4
                        and abs(position[1] - SCREEN_SIZE[1] / 2 + 32) // (64 * zoom)
                        <= 4
                        or "store" in TILE_ATTRIBUTES.get(machine_ui, ())
                    ):
                        world_x = (
                            location["tile"][0] * 16
                            + location["tile"][2]
                            + (position[0] - SCREEN_SIZE[0] / 2 + 32) // (64 * zoom)
                        )
                        world_y = (
                            location["tile"][1] * 16
                            + location["tile"][3]
                            + (position[1] - SCREEN_SIZE[1] / 2 + 32) // (64 * zoom)
                        )
                        grid_position = [
                            (world_x // 16, world_y // 16),
                            (world_x % 16, world_y % 16),
                        ]
                        if (
                            grid_position[1]
                            ...
                            while (
                                ...
                                if (
                                    ...
                                    grid_position = [
                                        ...
                                        (
                                            grid_position[1][0],
                                            (grid_position[1][1] - 1) % 16,
                                        ),
                                    ]
                        if event.button == 1:
                            machine_ui, chunks, location, machine_inventory = (
                                left_click(
                                    machine_ui,
                                    grid_position,
                                    chunks,
                            ...
                            chunks, location, machine_ui = right_click(
                                chunks,
                                grid_position,
                                inventory,
                                inventory_number,
                    ...
                    if event.button == 4 or event.button == 5:
                        inventory_number = (
                            inventory_number + (event.button == 5) - (event.button == 4)
                        ) % INVENTORY_SIZE
            ...
            health = chunks[location["room"]][
                (location["tile"][0], location["tile"][1])
            ][(location["tile"][2], location["tile"][3])].health
            max_health = chunks[location["room"]][
                (location["tile"][0], location["tile"][1])
            ][(location["tile"][2], location["tile"][3])].max_health
            ...
            camera = [
                ...
                - (
                    (
                        location["tile"][2] * TILE_SIZE
                        + location["tile"][0] * CHUNK_SIZE
                        + 32
                    )
                    * zoom
                ),
                ...
                - (
                    (
                        location["tile"][3] * TILE_SIZE
                        + location["tile"][1] * CHUNK_SIZE
                        + 32
                    )
                    * zoom
                ),
