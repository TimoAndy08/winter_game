...
def render(state: GameState, chunks, window, images) -> tuple:
    if state.menu_placement != "main_game":
        window = render_menu(state.menu_placement, state.save_file_name, state.controls, window, state.scroll, state.control_adjusted, state.world_type, state.seed, images)
        ...
        window = render_ui(
            ...
            state.inventory,
            state.machine_ui,
            chunks[state.location["opened"][0]].get(state.location["opened"][1], chunks[0, 0][0, 0]).get("recipe", 0),
            state.health,
            state.max_health,
