...
def animal(chunks, chunk, tile, current_tile, create_tiles, delete_tiles, location, inventory_key):
    if max(chunk[0] * 16 + tile[0] - location["tile"][0] * 16 - location["tile"][2], 
           chunk[1] * 16 + tile[1] - location["tile"][1] * 16 - location["tile"][3]) < 8 and inventory_key == ATTRACTION[current_tile["kind"]]:
