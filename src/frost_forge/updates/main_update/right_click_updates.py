...
def right_click(
    ...
    if "store" in ATTRIBUTES.get(machine_ui, ()):
        chunks, machine_inventory = open_storage(position, chunks, location, inventory, machine_ui, True)
    else:
        machine_ui = "game"
        ...
        if "kind" in mining_tile and mining_tile["kind"] not in UNBREAK:
            chunks, delete_mining_tile, mining_tile = break_tile(mining_tile["kind"], inventory, player_tile, chunks, mining_tile, inventory_number)
    ...
    return chunks, location, machine_ui, machine_inventory