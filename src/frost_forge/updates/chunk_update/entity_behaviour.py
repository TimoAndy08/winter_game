...
def move_entity(chunks, chunk, tile, current_tile, create_tiles, delete_tiles, type, location):
    obscured_path = False
    if "goal" not in current_tile:
        if type == 0:
            goal = (randint(-8, 8), randint(-8, 8))
            current_tile["goal"] = ((chunk[0] * 16 + (tile[0] + goal[0]) // 16, chunk[1] + (tile[1] + goal[1]) // 16), ((tile[0] + goal[0]) % 16, (tile[1] + goal[1]) % 16))
        elif type == 1:
            current_tile["goal"] = ((location["tile"][0], location["tile"][1]), (location["tile"][2], location["tile"][3]))
        goal_tile = chunks[current_tile["goal"][0]].get(current_tile["goal"][1], {})
        current_tile["path"] = []
        start = (chunk[0] * 16 + tile[0], chunk[1] * 16 + tile[1])
        goal = (current_tile["goal"][0][0] * 16 + current_tile["goal"][1][0], current_tile["goal"][0][1] * 16 + current_tile["goal"][1][1])
    ...
    if randint(0, 100) >= 0 and len(current_tile["path"]) > 0:
        path_tile = chunks[current_tile["path"][0][0]].get(current_tile["path"][0][1], {})
        if "kind" not in path_tile and ("floor" not in path_tile or "door" != FLOOR_TYPE.get(path_tile["floor"]) != "fluid"):
